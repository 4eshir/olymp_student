    ...
    <div class='section animate__animated animate__fadeIn'>
        ...
        @if ($model->email_verified_at == null)
            <form method="POST" class="verification_info" action="{{ route('verification.send') }}">
                @csrf
                <p class="verification_text"><b>Шаг 1. Подтвердите Ваш адрес электронной почты</b></p>
                <p>    На Вашу почту выслано письмо для подтверждения. Пожалуйста, откройте его и следуйте инструкции</p>
        ...
        @if ($model->phone_verified_at == null)
            <form method="POST" class="verification_info" action="{{ route('phoneConfirm') }}">
                ...
                <p class="verification_text"><b>Шаг 2. Подтвердите Ваш номер сотового телефона</b></p>
                <p>    Для подтверждения Вашего номера телефона, нажмите на кнопку ниже.</p>
                <input type="submit" class="btn btn-warning" value="Отправить СМС с кодом подтверждения">
                <p>    Если Вы неверно указали номер сотового телефона, пожалуйста, перейдите в раздел Контакты и укажите верные данные.</p>
            </form>
        ...
        @else
            ...
            @if (!$model->completed())
                <div class='verification_info'>
                    ...
                    <p>    Нажмите на иконку карандаша, чтобы открыть форму редактирования и заполнить данные.</p>
