    ...
    <div class='section animate__animated animate__fadeIn'>
        {{--@if(session('warning'))
            <div class="alert alert-success" style="width: 100%">
                {{ session('warning') }}
                {{ \Illuminate\Support\Facades\Cache::get("code:$phone") }}
                {{ \Illuminate\Support\Facades\Cache::has("code:$phone") }}
            </div>
        @endif--}}
        ...
        <form method="POST" class="verification_form" action="{{ route('phoneConfirmProcess') }}" id="confirmForm">
            @csrf
            <label style="width: 100%; margin-bottom: 15px">
                Номер телефона
                <input name="_phone" type="tel" readonly class="form-control" value="{{$phone}}"/>
                <input name="resend" type="submit" class="btn btn-link" id="timer" href="{{ route('phoneConfirm') }}" value="Отправить код повторно" style="display: none; padding: 0; margin: 0; font-size: 16px">
                <span type="submit" id="timerSpan" style="display: block">Отправить код повторно</span>
            </label>

            <label style="width: 100%">
                Введите код из СМС
                <div class="code-input">
                    <input name="_code[]" min="0" max="9" class="code-input-input" type="text" maxlength="1" autocomplete="off" />
                    <input name="_code[]" min="0" max="9" class="code-input-input" type="text" maxlength="1" autocomplete="off" />
                    <input name="_code[]" min="0" max="9" class="code-input-input" type="text" maxlength="1" autocomplete="off" />
                    <input name="_code[]" min="0" max="9" class="code-input-input" type="text" maxlength="1" autocomplete="off" />
                </div>
            ...
            <input type="submit" class="btn btn-primary" value="Подтвердить номер" style="margin-top: 20px; width: 100%">
        </form>
    </div>
...
<script>
    document.querySelectorAll('.code-input input').forEach((input, index) => {
        input.addEventListener('keyup', () => {
            if (input.value.length === 1 && index !== 3) {
                document.querySelectorAll('.code-input input')[index + 1].focus();
            }
        });
    });

    document.getElementById('confirmForm').addEventListener('keydown', function(event) {
    ...
    let remainingTime = localStorage.getItem('remainingTime') ? parseInt(localStorage.getItem('remainingTime')) : totalTimeInSeconds;

    // Функция для обновления таймера на странице
    function updateTimerDisplay() {
        const minutes = Math.floor(remainingTime / 60);
        const seconds = remainingTime % 60;
        document.getElementById('timerSpan').innerHTML = 'Отправить код повторно через ' + `${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
    }
    ...
    function countdown() {
        if (remainingTime > 0) {
            remainingTime--;
            localStorage.setItem('remainingTime', remainingTime); // Сохраняем текущее время в localStorage
    ...
    if (@json($refreshTimer) && localStorage.getItem(remainingTime) !== null) {
