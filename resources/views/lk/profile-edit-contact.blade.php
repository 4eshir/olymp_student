...
<div class='profile'>
    ...
    <div class='section animate__animated animate__fadeIn'>
        ...
        <form class='mainform_profile' method="POST" action="{{ route('profileRequestContact') }}">
            <input type="hidden" name="_token" value="{{ csrf_token() }}" />
            <input type="hidden" name="_id" value="{{ $model->id }}" />

            <div>
                <input name="email" type="email" class="form-control" id="FormControlInput1" placeholder="Email" value="{{ $model->email }}">
                <input name="phoneNumber" type="tel" class="form-control" id="FormControlInput2" placeholder="Номер телефона" value="{{ $model->phone_number }}">
            </div>
